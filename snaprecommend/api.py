...
def editorial_slices():
    ...
    return (
        flask.jsonify(
            [serialize_editorial_slice(slice) for slice in slices],
        ),
        200,
    )
...
def create_slice():
    ...
    try:
        ...
        return (
            flask.jsonify({"status": "failed", "error": "Slice cannot be created."}),
            500,
        )
...
@api_blueprint.route("/editorial_slice/<string:slice_id>", methods=["DELETE"])
@login_required
...
@api_blueprint.route("/editorial_slice/<string:slice_id>", methods=["POST"])
@login_required
...
@api_blueprint.route("/editorial_slice/<string:slice_id>/snaps", methods=["POST"])
@login_required
...
@api_blueprint.route("/editorial_slice/<string:slice_id>/remove_snap", methods=["POST"])
@login_required
