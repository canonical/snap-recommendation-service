...
def test_fill_rejects_multi_category_if_any_category_is_capped(app):
    ...
    game_only = _snap_obj("game", categories=[{"slug": "games"}])
    filled = _fill_slots(
        top3,
        reserved,
        [multi, game_only],
...
def test_select_records_history_and_updates_setting(
    ...
    with patch.dict(os.environ, {"FLASK_FEATURED_PUBLISHER_TOKEN": "fake-token"}):
        select_featured_snaps()
...
def test_select_without_token_skips_store_publish(
    ...
    with patch.dict(
        os.environ,
        {"FLASK_FEATURED_PUBLISHER_TOKEN": "", "FLASK_SNAPSTORE_MACAROON_KEY": ""},
    ):
        select_featured_snaps()

    # Nothing was published, so history must NOT be recorded either —
    # otherwise the cadence check would think a real run happened.
    rows = db.session.query(FeaturedHistory).filter_by(is_manual=False).all()
    assert len(rows) == 0
    ...
    from snaprecommend.settings import get_setting
    assert get_setting("featured_last_updated") is None
...
def test_select_logs_failed_pipeline_step_on_error(app):
